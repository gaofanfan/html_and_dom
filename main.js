...
function getFillScore2(id,answer) {
  ...
  for (var i = 0;i < id.length;i++) {
    var newAnswer = _.contains(newAnswers , document.getElementById(id[i]).value);
    if (!newAnswer) {
      newAnswers.push(document.getElementById(id[i]).value);
    }
  }
  for (var j = 0;j < newAnswers.length; j++) {
    if ( _.contains(answer, newAnswers[j])) {
      score += 10;
   }
 }
 return score;
